...
export const SOURCE_LABELS: Record<TopicSource, string> = { dogdrip: "Dogdrip", donga: "동아일보", hankyung: "한국경제", khan: "경향신문", mbc: "MBC 뉴스" };
export const SOURCE_ACCENTS: Record<TopicSource, string> = { dogdrip: "bg-lime-100 text-lime-900", donga: "bg-sky-100 text-sky-800", hankyung: "bg-emerald-100 text-emerald-800", khan: "bg-violet-100 text-violet-800", mbc: "bg-rose-100 text-rose-800" };
...
export function filterAndSortTopics(topics: TopicRecord[], source: SourceFilter, sort: TopicSort): TopicRecord[] {
  return topics.filter((topic) => source === "all" || topic.source === source).slice().sort((left, right) => compareTopics(left, right, sort));
}
